...
source("rcode.R")
#read data
original_data <- read.table(file = "anjala.csv", header = TRUE, sep = ",", col.names = 
                     c('Year', 'Month', 'Day', 'Hour', 'Time zone', 'Celcius'))
#setwd("C:/Users/vilma/OneDrive - Aalto University/Yliopisto/Kandi")
#original_data <- read.table(file = "anjaladata.csv", header = TRUE, sep= ",", 
...
original_data$Date <- as.Date(with(original_data, paste(Year,Month,Day,sep="-")),"%Y-%m-%d")
...
#   geom_hline(data = mean_10, aes(yintercept = mean_val_10)) + 
#   facet_grid(~Year, scales = "free_x")  + 
#   labs(title = "Vuorokauden ylin lämpötila Anjalassa", x = "Päivä (heinäkuu)",
#   y = "Lämpötila (\u00B0C)")
# 
...
#   labs(x = "Day (July)", y = "Temperature (\u00B0C)") + ylim(5,35)
# 
# grid.arrange(ggp1, ggp2, ncol = 2, top = 
#    "Maximum daily temperatures in July in Anjala")

#Decades
data_july_dec <- filter(data_july, Group == "1960" | Group == "1970" | 
                          Group == "1980" | Group == "1990" | Group == "2000" |
                          Group == "2010")
mean_byDecade <- data_july_dec%>% group_by(Group)%>%summarise(mean_val_dec = 
                                                                mean(Celcius))
x2 <- mean_byDecade$mean_val_dec
d2 <- mean(x2[2:6]-x2[1:5])
data_july_dec$h2 <- c(rep(x2[1:1],10*31), rep(x2[1:1]+d2,10*31), 
                      rep(x2[1:1]+2*d2,10*31), rep(x2[1:1]+3*d2,10*31), 
                      rep(x2[1:1]+4*d2,10*31), rep(x2[1:1]+5*d2,10*31))
...
#y_centered_d <- data_july_dec$Celcius-as.numeric(data_july_dec$h2)
y_centered_d <- as.numeric(data_july_dec$Celcius-c(rep(0,10*31), rep(d2,10*31),
...
# #suom. Decades
# data_july_dec <- filter(data_july, Group == "1960" | Group == "1970" | 
#   Group == "1980" | Group == "1990" | Group == "2000" |Group == "2010")
# mean_byDecade <- data_july_dec%>% group_by(Group)%>%summarise(mean_val_dec =
#    mean(Celcius))
# ggplot(data_july_dec, aes(Date, Celcius)) + geom_point(aes(colour=Year)) + 
#   scale_x_date(date_breaks = "4 years" , date_labels = "%Y") +
#   geom_hline(data = mean_byDecade, aes(yintercept = mean_val_dec), size=1) + 
#   facet_grid(~Group, scales = "free_x", labeller = labeller(Group = 
#     c('1960' = "1960-luku",'1970'="1970-luku",'1980'="1980-luku",'1990'=
#     "1990-luku",'2000'="2000-luku", '2010'="2010-luku"))) + 
#   labs(title = "Heinäkuun vuorokausien ylin lämpötila Anjalassa vuodesta 
#     1960 alkaen", x = "Päivämäärä", y = "Lämpötila (\u00B0C)")


# 10 year periods from 1961
data_july$Group2 <- c(rep(0,3*31), rep(1962,10*31), rep(1972,10*31), 
                      rep(1982,10*31), rep(1992,10*31), rep(2002,10*31), 
                      rep(2012,10*31))
data_jul_10 <- filter(data_july, Group2 == "1962" | Group2 == "1972" |
                        Group2 == "1982" | Group2 == "1992" | 
                        Group2 == "2002" |Group2 == "2012")
mean_by_10 <- data_jul_10%>% group_by(Group2)%>%summarise(mean_val_10 = 
                                                            mean(Celcius))
x <- mean_by_10$mean_val_10
d <- mean(x[2:6]-x[1:5])
data_jul_10$h <- c(rep(x[1:1],10*31),rep(x[1:1]+d,10*31), rep(x[1:1]+2*d,10*31),
                   rep(x[1:1]+3*d,10*31), rep(x[1:1]+4*d,10*31),
                   rep(x[1:1]+5*d,10*31))
...
y_centered <- as.numeric(data_jul_10$Celcius-c(rep(0,10*31), rep(d,10*31), 
...
# l_d <- length(y_centered_d)
# 
# values_sigma_m <- rep(0,l_d-1)
# for (k in 1:(l_d-1)) {
#   values_sigma_m[k] <- sigma_m(y_centered_d, l_d, k)
# }
# 
...
# for (k in 1:(500)) {
#   values_p_estim2[k] <- p_estim(y_centered_d, l_d, k, 36, 6*d2)
# }
...
# for (k in 1:(1500)) {
#   values_p_estim[k] <- p_estim_zero(y_centered, l, k, 35, 6*d)
# }
# plot(values_p_estim, type = 'l', xlab = "k", ylab = "p estim.", main = 
#        "Probability for T=45 \u00B0C when gamma=0")
# 
# # 
# values_p_estim <- rep(0,1000)
# for (k in 1:(1000)) {
#   values_p_estim[k] <- p_estim_zero(y_centered_d, l_d, k, 45, 6*d2)
# }
# plot(values_p_estim, type = 'l', xlab = "k", ylab = "p estim.", main = 
#        "Probability for T=45 \u00B0C when gamma=0")
